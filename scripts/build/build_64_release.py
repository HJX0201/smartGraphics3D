#!/usr/bin/env python3
"""Build, test, and deploy smartGraphics3D for Windows x64 Release."""
...
def deploy_qt(qt_root: Path, executable: Path, environment: dict[str, str]) -> None:
    ...
    succeeded = False
    if deploy.is_file():
        command = [str(deploy), "--release", "--no-translations", str(executable)]
        print("+", subprocess.list2cmdline(command), flush=True)
    ...
    platform = executable.parent / "platforms" / "qwindows.dll"
