...
- 修改前后检查 Git 工作区，只处理当前需求范围内的文件，不覆盖或夹带无关改动。
- 提交内容必须职责单一、范围合理；提交说明应准确描述本次完成的功能或需求。
- 每完成一个独立功能、需求或项目阶段，先执行与改动相称的最小有效测试。
- 简单测试只是提交前的最低要求。涉及自研 C++、新增功能或发布时，仍须满足上方强制规则和提交门槛。
- 测试失败、功能未完成或存在已知错误时不得提交；先修复并重新验证。
- 测试成功后，向开发者打印本次改动对应的、可直接执行的 Git 暂存和提交指令，并给出合理的提交说明。
- Agent 不得自行执行 `git add`、`git commit` 或 `git push`；Git 提交和推送由开发者亲自完成。
- 打印的 Git 指令必须只包含当前需求涉及的明确文件，不得使用可能夹带无关改动的全量暂存指令。
- 不使用破坏性 Git 操作清除他人或用户改动，不擅自改写已共享的提交历史。
