...
- 测试成功后，默认打印仅暂存本次明确文件的 Git 指令和准确提交说明，禁止全量暂存。
- 用户明确要求 Agent 代为提交或推送时，Agent 可以仅暂存本次明确文件，在检查暂存差异和提交目标后执行 `git commit` 和普通 `git push`。
- `git add -A`、`git add .`、强制推送、删除远程分支或标签、改写共享历史等扩大范围或破坏性操作，必须由用户单独明确确认；强制推送优先使用带精确旧 SHA 的 `--force-with-lease`。
